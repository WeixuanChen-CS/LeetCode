-- 【题目】❌197. Rising Temperature (Easy)
...
SELECT id 
FROM (
    SELECT id, temperature,LAG(temperature) OVER (ORDER BY id) AS pre
    FROM Weather
)t
WHERE pre>temperature
```
...
FROM (
    ...
    LAG(temperature) OVER (ORDER BY recordDate) AS pre_t,
    LAG(recordDate) OVER (ORDER BY recordDate) AS pre_d
    FROM Weather
)t
WHERE temperature>pre_t
AND DATEDIFF(recordDate,pre_d)=1
```
