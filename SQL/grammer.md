...
49. SUBSTRING(字段名, 2)
==================================================
SUBSTRING(字段名, 2)：从第 2 个字符开始截取到最后：
...
跳过前 1 条，再取 1 条
LIMIT 取几条 OFFSET 跳过几条

==================================================
54. GROUP_CONCAT()
==================================================
GROUP_CONCAT()：分组之后，把同一组里的多个值，默认用逗号连接成一个字符串
但是也可以指定分隔符，用SEPARATOR，比如如果你想逗号后面加空格：
GROUP_CONCAT(product SEPARATOR ', ')

==================================================
55. 正则表达式
==================================================
REGEXP_LIKE(mail, 正则, 'c')
固定域名：^____@leetcode\\.com$
第一个必须字母：^[A-Za-z]____@leetcode\\.com$
后面允许哪些字符：^[A-Za-z][A-Za-z0-9_.-]*@leetcode\\.com$
'c'表示：case-sensitive，区分大小写
